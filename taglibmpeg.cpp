...
zend_object_value taglibmpegfile_create_handler(zend_class_entry *type
                                                    TSRMLS_DC) {
  zval *tmp;
  zend_object_value retval;

  taglibmpegfile_object *obj =
      (taglibmpegfile_object *)emalloc(sizeof(taglibmpegfile_object));
...
#endif
  retval.handle = zend_objects_store_put(obj, NULL, taglibmpegfile_free_storage,
                                         NULL TSRMLS_CC);
...
PHP_METHOD(TagLibMPEG, __construct) {
  ...
  taglibmpegfile_object *thisobj =
      (taglibmpegfile_object *)zend_object_store_get_object(getThis()
                                                            TSRMLS_CC);

  thisobj->frameFactory = TagLib::ID3v2::FrameFactory::instance();
  try {
    thisobj->file = new TagLib::MPEG::File((TagLib::FileName)filestr,
                                           thisobj->frameFactory, true);
...
PHP_METHOD(TagLibMPEG, getAudioProperties) {
  ...
  add_assoc_bool(return_value, "protectionEnabled",
                 (zend_bool)audioProperties->protectionEnabled());
  add_assoc_bool(return_value, "isCopyrighted",
                 (zend_bool)audioProperties->isCopyrighted());
  add_assoc_bool(return_value, "isOriginal",
                 (zend_bool)audioProperties->isOriginal());
}
...
PHP_METHOD(TagLibMPEG, setID3v1) {
  ...
  for (zend_hash_internal_pointer_reset_ex(hIndex, &pointer);
    ...
    if (key_type != HASH_KEY_IS_STRING) {
      php_error(
          E_WARNING,
          "TagLibMPEG::setID3v1 expects associative array of string values!");
      RETURN_FALSE;
      break;
    }
    ...
    TagLib::StringList *destValue =
        new TagLib::StringList(*(new TagLib::String(Z_STRVAL_PP(data))));
    ...
    if (!propMap.insert(*destKey, *destValue) || taglib_error()) {
      php_error(E_WARNING,
                "PropertyMap::insert() failed, possibly invalid key provided.");
      break;
    }
  }
  ...
  if (thisobj->file->save(TagLib::MPEG::File::TagTypes::ID3v1)) {
    if (failedToSet.begin() == failedToSet.end()) {
      ...
      for (TagLib::Map<TagLib::String, TagLib::StringList>::Iterator property =
               failedToSet.begin();
           property != failedToSet.end(); property++) {
        add_assoc_string(return_value, property->first.toCString(),
                         (char *)(property->second.toString().toCString()), 1);
      }
    }
...
PHP_METHOD(TagLibMPEG, stripTags) {
  taglibmpegfile_object *thisobj =
      (taglibmpegfile_object *)zend_object_store_get_object(getThis()
                                                            TSRMLS_CC);
  /**
   * NOTE: calling MPEG::File::strip() with other arguments failed to remove any
   * tags from file
   */
 ...
 */
static bool id3v2_set_frame(TagLib::ID3v2::Tag *tag, zval **data,
                            TagLib::ByteVector byteVector, char *frameID) {
  /**
    * this abstraction would make things much, much nicer:
    * TagLib::ID3v2::Frame *newFrame =
    *thisobj->frameFactory->createFrame(byteVector);
    *
   ...
   */
  case "APIC"_CASE: {
    const char *genericWarning =
        "AttachedPictureFrame expects a specific array argument, e.g.: \n\
    [\n\
    ...
    TagLib::ID3v2::AttachedPictureFrame *pictureFrame =
        new TagLib::ID3v2::AttachedPictureFrame(byteVector);
    ...
    if (zend_hash_find(pictureArray, "data", 5, (void **)&apic_data) ==
        SUCCESS) {
      ...
      unsigned char *b64data =
          php_base64_decode((const unsigned char *)Z_STRVAL_PP(apic_data),
                            Z_STRLEN_PP(apic_data), &decsize);
      TagLib::ByteVector dataVector = TagLib::ByteVector::fromCString(
          (const char *)b64data, (size_t)decsize);
      pictureFrame->setPicture(dataVector);
    } else if (zend_hash_find(pictureArray, "file", 5, (void **)&apic_file) ==
               SUCCESS) {
    ...
    if (zend_hash_find(pictureArray, "mime", 5, (void **)&apic_mime) ==
        SUCCESS) {
    ...
    if (zend_hash_find(pictureArray, "type", 5, (void **)&apic_type) ==
        SUCCESS) {
      ...
      if (type >= 0x0 && type <= 0x14) {
        ...
        php_error(E_WARNING, "Invalid value for AttachedPictureFrame::Type. "
                             "Try using TagLib::APIC_* constants");
        return false;
      }
    } else {
      php_error(E_WARNING, genericWarning);
    ...
    if (zend_hash_find(pictureArray, "desc", 5, (void **)&apic_desc) ==
        SUCCESS) {
  ...
  case "TDRC"_CASE: {
    ...
    TagLib::ID3v2::TextIdentificationFrame *newFrame =
        new TagLib::ID3v2::TextIdentificationFrame(byteVector);
   ...
   */
  case "TXXX"_CASE: {
    TagLib::ID3v2::UserTextIdentificationFrame *newFrame =
        new TagLib::ID3v2::UserTextIdentificationFrame(byteVector);

    const char *genericWarning =
        "TXXX aka UserTextIdentificationFrame requires an array argument e.g.\n\
    [\n\
  ...
  case "COMM"_CASE: {
    ...
    TagLib::ID3v2::CommentsFrame *newFrame =
        new TagLib::ID3v2::CommentsFrame(byteVector);
    TagLib::String *frametext = new TagLib::String(Z_STRVAL_P(*data));
  ...
  case "WPUB"_CASE: {
    ...
    TagLib::ID3v2::UrlLinkFrame *newFrame =
        new TagLib::ID3v2::UrlLinkFrame(byteVector);
   ...
   */
  case "WXXX"_CASE: {
    TagLib::ID3v2::UserUrlLinkFrame *newFrame =
        new TagLib::ID3v2::UserUrlLinkFrame(byteVector);
    const char *genericWarning =
        "WXXX aka UserUrlLinkFrame requires an array argument e.g. \n\
    [\n\
   ...
   */
  case "OWNE"_CASE: {
    TagLib::ID3v2::OwnershipFrame *newFrame =
        new TagLib::ID3v2::OwnershipFrame(TagLib::String::Type::UTF8);
    const char *genericWarning =
        "OWNE aka OwnershipFrame requires an array argument e.g. \n\
    [\n\
        'date' => '19691231',\n\
        'paid' => '$0.99',\n\
        'seller' => 'someone'\n\
  ...
  case "PRIV"_CASE: {
    ...
    const char *genericWarning =
        "PRIV aka PrivateFrame requires an array argument e.g. \n\
    [\n\
    ...
    if (zend_hash_find(txxxArray, "data", 5, (void **)&privdata) == SUCCESS) {
      ...
      int decsize;
      unsigned char *b64data =
          php_base64_decode((const unsigned char *)Z_STRVAL_PP(privdata),
                            Z_STRLEN_PP(privdata), &decsize);
      TagLib::ByteVector dataVector = TagLib::ByteVector::fromCString(
          (const char *)b64data, (size_t)decsize);
   ...
   */
  case "UFID"_CASE: {
    TagLib::ID3v2::UniqueFileIdentifierFrame *newFrame =
        new TagLib::ID3v2::UniqueFileIdentifierFrame(byteVector);
    const char *genericWarning =
        "UFID aka UniqueFileIdentifierFrame requires an array argument e.g. \n\
    [\n\
    ...
    if (zend_hash_find(txxxArray, "id", 3, (void **)&id) == SUCCESS) {
      ...
      TagLib::ByteVector whyIsThisAByteVector =
          TagLib::ByteVector::fromCString((const char *)Z_STRVAL_PP(id));
  ...
  case "USLT"_CASE: {
    ...
    TagLib::ID3v2::UnsynchronizedLyricsFrame *newFrame =
        new TagLib::ID3v2::UnsynchronizedLyricsFrame(byteVector);
    /**
  ...
  case "ENCR"_CASE: {
    php_error(
        E_WARNING,
        "Audio Encryption not supported (take your DRM bullshit elsewhere)");
    return false;
  } break;
  ...
  case "IPLS"_CASE: {
    php_error(E_DEPRECATED,
              "Please use TIPL in place of IPLS for Involved People List.");
    return false;
  } break;
  ...
  case "RBUF"_CASE: {
    php_error(E_WARNING,
              "Recommended Buffer Size (streaming) Frame not supported.");
    return false;
  } break;
  ...
  case "RVAD"_CASE: /* relative volume adjustment (replay gain) */
  {
    php_error(
        E_WARNING,
        "Frames intended for media playback (synchronized time codes or lyrics for karaoke players; popularimeter ratings, etc)\
...
PHP_METHOD(TagLibMPEG, setID3v2) {
  ...
  zend_bool overwrite_existing_tags = true;
  if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "z|l", &newFrames,
                            &overwrite_existing_tags) == FAILURE) {
    RETURN_FALSE;
  }

  if (Z_TYPE_P(newFrames) != IS_ARRAY) {
    php_error(E_WARNING, "TagLibMPEG::setID3v2 expects associative array of "
                         "FRAME_IDs as keys. See "
  ...
  for (zend_hash_internal_pointer_reset_ex(hIndex, &pointer);
    ...
    ulong index;
    index_type = zend_hash_get_current_key_ex(hIndex, &frameID, &frameID_length,
                                              &index, 0, &pointer);
    ...
    const TagLib::ByteVector byteVector =
        TagLib::ByteVector::fromCString(frameID, frameID_length);

    if (overwrite_existing_tags) {
      TagLib::ID3v2::FrameList l = tag->frameListMap()[frameID];
      if (!l.isEmpty()) {
        for (TagLib::List<TagLib::ID3v2::Frame *>::Iterator it = l.begin();
             it != l.end(); it++) {
  ...
  //    const TagLib::StringList unsupported =
  //    tag->properties().unsupportedData();
...
static zend_function_entry php_taglibmpeg_methods[] = {
    ...
    PHP_ME(TagLibMPEG, stripTags, NULL, ZEND_ACC_PUBLIC){NULL, NULL, NULL}};
